...
const heroProduct = 'https://images.unsplash.com/photo-1546435770-a3e426bf472b?auto=format&fit=crop&w=1100&q=90'
      ...
      <div className="relative isolate overflow-hidden rounded-[28px] bg-[#dcebdd] px-6 py-9 sm:px-10 lg:min-h-[520px] lg:px-16 lg:py-14">
        ...
        <div className="relative z-10 max-w-xl lg:pt-5">
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <button type="button" onClick={onShopNow} className="group inline-flex h-12 items-center gap-3 rounded-full bg-[#17211d] px-6 text-sm font-bold text-white transition hover:-translate-y-0.5 hover:bg-[#ff5c35] hover:shadow-xl">
              Shop collection <ArrowRight size={17} className="transition group-hover:translate-x-1" />
            </button>
            <button type="button" className="inline-flex h-12 items-center gap-2 rounded-full border border-[#17211d]/15 bg-white/45 px-5 text-sm font-bold text-[#17211d] transition hover:bg-white">
              <span className="grid size-7 place-items-center rounded-full bg-white"><Play size={12} fill="currentColor" /></span> Watch the story
            </button>
          </div>
        ...
        <div className="relative mt-9 h-[280px] lg:absolute lg:bottom-0 lg:right-0 lg:mt-0 lg:h-full lg:w-[52%]">
          <div className="absolute left-3 top-3 z-10 rounded-2xl bg-white/90 p-3 shadow-xl backdrop-blur sm:left-8 sm:top-14">
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Editor’s pick</p>
            <p className="mt-1 text-sm font-extrabold text-slate-900">Sonic Pulse Pro</p>
            <p className="mt-0.5 text-xs font-bold text-[#ff5c35]">From ₹8,999</p>
          </div>
          <img src={heroProduct} alt="Premium wireless headphones" className="absolute bottom-[-70px] right-[-40px] h-[370px] w-[520px] rotate-[-8deg] object-contain mix-blend-multiply drop-shadow-[0_35px_30px_rgba(23,33,29,0.28)] sm:right-4 lg:bottom-[-35px] lg:right-[-30px] lg:h-[560px] lg:w-[680px]" />
          <span className="absolute bottom-8 right-5 rounded-full bg-[#ffb000] px-4 py-3 text-center text-[10px] font-extrabold uppercase leading-3 text-slate-950 shadow-lg sm:right-12 lg:bottom-14 lg:right-16">Save<br/><span className="text-base">25%</span></span>
        </div>
      </div>
    </section>
  )
