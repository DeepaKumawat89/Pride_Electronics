  ...
  return (
    <article className="group flex min-w-0 flex-col">
      <div className="relative aspect-[1/0.9] overflow-hidden rounded-[26px] bg-white">
        ...
        <span className="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1.5 text-[9px] font-extrabold uppercase tracking-wider text-slate-800 backdrop-blur">{product.badge}</span>
        <button type="button" onClick={() => onLike(product.id)} className={`absolute right-3 top-3 grid size-9 place-items-center rounded-full backdrop-blur transition ${liked ? 'bg-[#ff5c35] text-white' : 'bg-white/90 text-slate-600 hover:text-[#ff5c35]'}`} aria-label={liked ? 'Remove from wishlist' : 'Add to wishlist'}><Heart size={16} fill={liked ? 'currentColor' : 'none'} /></button>
        <div className="absolute inset-x-3 bottom-3 flex translate-y-4 gap-2 opacity-0 transition duration-300 group-hover:translate-y-0 group-hover:opacity-100">
          <button type="button" onClick={() => onView(product)} className="grid size-11 place-items-center rounded-full bg-white text-slate-900 shadow-lg transition hover:bg-slate-100" aria-label="Quick view"><Eye size={17} /></button>
