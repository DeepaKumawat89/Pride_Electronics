...
export default function ProductReviewsPage({ product, onBack }) {
  return (
        ...
        <div className="mb-6 grid gap-5 md:grid-cols-[minmax(0,1.35fr)_minmax(280px,.65fr)]">
          <header className="relative overflow-hidden rounded-[28px] bg-[#dcebdd] px-5 py-7 sm:px-8 sm:py-9">
            ...
            <div className="relative flex h-full flex-col justify-between gap-6">
              ...
              <div className="flex w-fit items-center gap-3 rounded-2xl bg-white/75 px-4 py-3 shadow-sm backdrop-blur"><span className="grid size-10 place-items-center rounded-xl bg-[#397a4a] text-white"><MessageSquareText size={18} /></span><div><p className="text-lg font-extrabold text-slate-950">{customerReviews.length}</p><p className="text-[9px] font-bold uppercase tracking-wider text-slate-500">Featured reviews</p></div></div>
            </div>
          </header>
          <RatingSummary product={product} showTitle />
        </div>

        <ProductReviews product={product} showAll reviewsOnly />
      </div>
    </div>
  )
}
