    ...
    <footer className="bg-[#17211d] text-white">
      <div className="mx-auto max-w-[1440px] px-4 py-12 sm:px-6 lg:px-10 lg:py-16">
        <div className="grid gap-12 border-b border-white/10 pb-12 lg:grid-cols-[1.3fr_2fr]">
          ...
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-3">{columns.map((column) => <div key={column.title}><h3 className="text-xs font-extrabold uppercase tracking-[0.16em] text-white/35">{column.title}</h3><ul className="mt-5 space-y-3">{column.links.map((link) => <li key={link}><button type="button" onClick={link === 'Sell with us' ? onBeSellerClick : undefined} className="text-xs font-semibold text-white/65 transition hover:text-white">{link}</button></li>)}</ul></div>)}</div>
        </div>
        ...
        <div className="flex flex-col gap-3 pt-7 text-[10px] font-semibold text-white/35 sm:flex-row sm:items-center sm:justify-between"><p>© 2026 Pride Electronics India Pvt. Ltd.</p><div className="flex gap-5"><a href="#">Privacy</a><a href="#">Terms</a><a href="#">Accessibility</a></div></div>
      </div>
    </footer>
  )
}
